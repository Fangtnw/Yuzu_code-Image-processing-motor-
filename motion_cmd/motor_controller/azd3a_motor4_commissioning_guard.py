"""Feedback-relative, tightly bounded commissioning guard for Motor 4."""
...
PUBLIC_TOPIC = "/motor4_commissioning/offset_mm"
...
class Motor4CommissioningGuard(Node):
    def __init__(self) -> None:
        super().__init__("azd3a_motor4_commissioning_guard")
        self.origin = None
    ...
    def on_joint_state(self, message: JointState) -> None:
        ...
        if not self.operator_command_received:
            # Follow live feedback until the operator acts. EtherCAT can
            # briefly publish zero before the slave reaches OP, so that first
            # sample must never become a stale relative origin.
    ...
    def on_command(self, message: Float64) -> None:
        ...
        if not math.isfinite(offset_mm):
            self.get_logger().error("REJECTED Motor 4 offset: expected a finite value")
            return
        if not -MAX_OFFSET_MM <= offset_mm <= MAX_OFFSET_MM:
            self.get_logger().error(
                "REJECTED Motor 4 offset: permitted range is -0.500..0.500 mm"
            )
            return
        increments = round(offset_mm / MIN_INCREMENT_MM)
        if abs(offset_mm - increments * MIN_INCREMENT_MM) > 1e-9:
            self.get_logger().error("REJECTED Motor 4 offset: use 0.001 mm increments")
            return
        if self.origin is None or self.command_position is None:
            self.get_logger().error("REJECTED Motor 4 offset: no valid feedback yet")
            return
        self.operator_command_received = True
        self.target_position = self.origin + offset_mm / 1000.0
        self.get_logger().info(f"Accepted Motor 4 startup-relative offset {offset_mm:.3f} mm")
