"""Feedback-relative, tightly bounded commissioning guard for Motor 5."""
...
PUBLIC_TOPIC = "/motor5_commissioning/offset_deg"
...
class Motor5CommissioningGuard(Node):
    def __init__(self) -> None:
        super().__init__("azd3a_motor5_commissioning_guard")
        ...
        self.get_logger().info(
            "Motor 5 commissioning guard ready: startup-relative -10..10 degrees, "
            "5 rpm, 5 rpm/s"
        )
    ...
    def on_command(self, message: Float64) -> None:
        ...
        if not math.isfinite(offset_deg) or not -MAX_OFFSET_DEG <= offset_deg <= MAX_OFFSET_DEG:
            self.get_logger().error("REJECTED Motor 5 offset: permitted range is -10..10 degrees")
            return
        if self.origin is None or self.command_position is None:
            self.get_logger().error("REJECTED Motor 5 offset: no valid feedback yet")
            return
        self.operator_command_received = True
        self.target_position = self.origin + math.radians(offset_deg)
        self.get_logger().info(f"Accepted Motor 5 startup-relative offset {offset_deg:.3f} deg")
